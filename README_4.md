...
Firing consumes the target's lock. After the Gunner fires, the Captain rolls a Battle Damage Assessment (Diplomacy) — the roll determines how much of the lock is retained and grants one fire correction:
...
### Power Core Actions (via Sensor Radar)

These consume the Captain's assigned Power Core in addition to the AP cost.
